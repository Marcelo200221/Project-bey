...
<template>
    <header class="header">
        ...
        <nav class="opciones">
            ...
            <template v-if="user">
                ...
                >
                    ...
                    >
                        ...
                        <!-- Suggestions / Reports Section (FOR NORMAL LOGGED IN USERS) -->
                        <template v-else>
                            <div class="px-3 py-1 text-[11px] font-extrabold uppercase tracking-wider text-amber-500/80">
                                Sugerencias
                            </div>
            ...
            <!-- GUEST NAV (UNAUTHENTICATED USER) -->
            <template v-else>
                ...
                <div class="hidden md:flex items-center gap-2">
                    ...
                    <router-link 
                        :to="{name: 'Iniciar'}" 
                        class="px-3.5 py-1.5 bg-white/15 hover:bg-white/25 text-white rounded-xl text-xs font-bold transition-all no-underline"
                    >
                        Iniciar Sesión
                    </router-link>
                    <router-link 
                        :to="{name: 'Registro'}" 
                        class="px-3.5 py-1.5 bg-slate-900/80 hover:bg-slate-900 text-amber-400 hover:text-amber-300 border border-amber-500/50 rounded-xl text-xs font-extrabold transition-all no-underline shadow-md"
                    >
                        Registrarse
                    </router-link>
                </div>

                <!-- Mobile Hamburger Button for Guests -->
                <button 
                    type="button" 
                    @click="open = !open" 
                    class="flex md:hidden items-center justify-center w-10 h-10 bg-white/10 hover:bg-white/20 text-white rounded-xl transition-all border border-white/20 shadow-sm active:scale-95 cursor-pointer z-50 ml-2"
                    aria-label="Abrir menú"
                >
                    <span class="text-lg font-bold">{{ open ? '✕' : '☰' }}</span>
                </button>
                ...
                <div 
                    v-if="open" 
                    @click="open = false" 
                    class="fixed inset-0 bg-black/50 backdrop-blur-xs z-40 transition-opacity md:hidden"
                ></div>
                ...
                >
                    <div 
                        v-if="open" 
                        class="fixed top-16 right-3 sm:right-6 w-72 max-w-[calc(100vw-1.5rem)] max-h-[calc(100vh-5rem)] bg-slate-900/95 border border-slate-700/80 shadow-2xl rounded-2xl p-4 backdrop-blur-xl z-50 flex flex-col gap-1 overflow-y-auto text-slate-100 md:hidden"
                    >
                        <div class="px-3 py-1 text-[11px] font-extrabold uppercase tracking-wider text-amber-500/80">
                            Navegación Pública
                        </div>
                        <router-link @click="open = false" class="block px-3.5 py-2 text-slate-200 hover:text-amber-400 hover:bg-white/10 rounded-xl transition-colors text-sm font-semibold flex items-center gap-2.5" :to="{name: 'Home'}">
                            <span>🏠</span> Inicio
                        </router-link>
                        <router-link @click="open = false" class="block px-3.5 py-2 text-slate-200 hover:text-amber-400 hover:bg-white/10 rounded-xl transition-colors text-sm font-semibold flex items-center gap-2.5" :to="{name: 'Personajes'}">
                            <span>👥</span> Personajes
                        </router-link>
                        <router-link @click="open = false" class="block px-3.5 py-2 text-slate-200 hover:text-amber-400 hover:bg-white/10 rounded-xl transition-colors text-sm font-semibold flex items-center gap-2.5" :to="{name: 'Home', query: {seasons: 'true'}}">
                            <span>🌀</span> Beyblades
                        </router-link>
                        <router-link @click="open = false" class="block px-3.5 py-2 text-slate-200 hover:text-amber-400 hover:bg-white/10 rounded-xl transition-colors text-sm font-semibold flex items-center gap-2.5" :to="{name: 'Settings'}">
                            <span>⚙️</span> Settings
                        </router-link>
                        ...
                        <router-link @click="open = false" class="block px-3.5 py-2 text-amber-400 hover:text-amber-300 hover:bg-white/10 rounded-xl transition-colors text-sm font-bold flex items-center gap-2.5" :to="{name: 'Iniciar'}">
                            <span>🔑</span> Iniciar Sesión
                        </router-link>
                        <router-link @click="open = false" class="block px-3.5 py-2 text-slate-100 hover:text-amber-300 hover:bg-white/10 rounded-xl transition-colors text-sm font-bold flex items-center gap-2.5" :to="{name: 'Registro'}">
